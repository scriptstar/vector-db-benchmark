...
const dbKeys = ["qdrant", "milvus", "weaviate", "pinecone", "sqlite", "topk", "chroma"];
...
function renderDBResults(db, data) {
  console.log(`Rendering results for ${db}:`, data);
  console.log(`DOM elements for ${db}:`, {lat: latEls[db], list: listEls[db], err: errEls[db]});
  
  if (!data) return;
  if (!data.ok) {
    if (errEls[db]) errEls[db].textContent = data.error || "Error";
    return;
  }
  if (latEls[db]) latEls[db].textContent = `${data.latency_ms?.toFixed(1)} ms`;
  const items = data.results || [];
  if (listEls[db]) listEls[db].innerHTML = items
    .map((it) => {
