...
def get_db(name: str) -> Any:
    ...
    if name == "topk":
        return TopKClient()
    if name == "sqlite":
        default_db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "music_vectors.db")
        return SQLite(db_path=os.getenv("SQLITE_DB_PATH", default_db_path))
    if name == "chroma":
        return ChromaDB(
...
class SearchRequest(BaseModel):
    query: str
    topk: int = 10
    dbs: List[str] = ["qdrant", "milvus", "weaviate", "pinecone", "topk", "sqlite", "chroma"]
...
def _startup_warmup():
    ...
    dbs = os.getenv("UI_WARMUP_DBS", "qdrant,milvus,weaviate,pinecone,topk,sqlite,chroma").split(",")
